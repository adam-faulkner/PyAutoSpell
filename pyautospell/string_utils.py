...
class StringUtils(object):
    ...
    def should_not_check_single_char(self, c):
        return c not in  string.ascii_lowercase+string.ascii_uppercase

    def should_not_check_string(self, w):
        lowerw = w.lower()
        if w in self.puncts:
            return True
    ...
    def __has_non_check_prefix(self, w):
    ...
    def __is_non_check_words(self, w):
        if w in self.non_check_words:
            return True
        return False

    def contains_no_check_puncts(self, w):
        for c in list(w):
            if c in self.not_check_if_contains_punct:
                return True
        return False

    def __is_all_punct(self, w):
